...
# Define the motor pins, set PWM frequency, set duty cycle, keep track of current motor action
...
# Define the motor actions like stop, bwd, fwd, turn

def motor_shutdown():                # for easier programming, full shutdown of the motors by deinitializing the pwm setup for that pin (deinit) and making them inputs (no signal coming from them)
    motor_stop()                      
...
def stop_scan():
    ...
    while stop_reading is None:              # if the reading is None (failed reading), keep trying to take the reading until it's not None
        time.sleep(0)                     # short delay to let echo clear
...
try:
    ...
    while True:
        ...
        time.sleep(0.005) # small 0.005 second pause in the main loop to keep the Pico stable
